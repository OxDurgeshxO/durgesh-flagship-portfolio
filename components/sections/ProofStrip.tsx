...
export default function ProofStrip() {
  return (
    <section aria-label="Professional Proof & Quick Signals" className="relative z-20 -mt-8 sm:-mt-12 px-4 sm:px-6 max-w-7xl mx-auto mb-16">
      <div className="glass rounded-2xl border border-border p-4 sm:p-6 shadow-2xl shadow-black/40 backdrop-blur-xl bg-[#0c0c14]/85">
          ...
          {PROOF_ITEMS.map((item, idx) => {
            ...
            const content = (
              <div className="flex flex-col gap-1.5 pt-3 sm:pt-0 sm:px-3 first:px-0">
                <div className="flex items-center gap-2 text-muted-foreground text-xs font-mono tracking-wider uppercase">
                  <Icon className="size-3.5 text-primary shrink-0" />
                  <span className="truncate">{item.label}</span>
                </div>
                <div className="text-ink text-xs sm:text-sm font-semibold tracking-tight flex items-center gap-1 group">
                  <span dangerouslySetInnerHTML={{ __html: item.value }} />
                  {item.href && (
                    <ArrowUpRight className="size-3 text-muted-foreground group-hover:text-primary group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all shrink-0" />
                  )}
                </div>
              </div>
            )
