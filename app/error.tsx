...
export default function ErrorBoundary({
  ...
  useEffect(() => {
    // Log sanitized error indicator without dumping raw traces to visitor
    console.error('Unhandled interface fault occurred', error?.digest || 'no-digest')
  }, [error])

  return (
    <div className="min-h-screen bg-[var(--bg-primary,#0c0c14)] text-slate-100 flex items-center justify-center px-6 relative selection:bg-purple-500/30">
      <div className="max-w-lg w-full text-center py-16">
        ...
        <div className="flex flex-wrap gap-3.5 justify-center items-center">
          <button
            onClick={() => reset()}
            className="px-6 py-3 rounded-xl bg-gradient-to-r from-purple-600 to-rose-500 text-white font-semibold hover:opacity-90 transition-all inline-flex items-center gap-2 shadow-lg shadow-purple-500/20 text-sm cursor-pointer"
          >
